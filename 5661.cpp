...
输入格式
	...
	第 i 行的第 1 个整数代表第 i 条记录乘坐的交通工具，0 代表地铁，1 代表公交车；
	第 2 个整数代表第 i 条记录乘车的票价 pricei ；
	第三个整数代表第 i 条记录开始乘车的时间 ti （距 0 时刻的分钟数）。
...
int main(){
	...
	for(int i = 0;i < n;i++){
		...
		if(b[i] == 1){ //判断类型
			for(int j = m;j < i;j++){ //枚举优惠
				if((t[i] - t[j]) <= 45 && b[j] == 0 && p[i] <= p[j]){ //判断条件，进行优惠
					b[j] = 1; //取消优惠
					f = 1; //优惠
					break; //退出枚举
				}else if((t[i] - t[j]) > 45){ //记录有效优惠范围
					m = j + 1; //更改范围
				}
			}
		}
		if(f) continue; //查看是否优惠
		ans += p[i]; //记录答案
	}
	cout << ans; //输出答案
	return 0;
